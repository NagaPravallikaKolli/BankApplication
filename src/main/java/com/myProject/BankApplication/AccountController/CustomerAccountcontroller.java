...
public class CustomerAccountcontroller {
    ...
    // public ResponseEntity<String> addAccount(@PathVariable int custid, @RequestBody Bankaccount accountDetails) {

    //     if (customerRepository.existsById(custid)) {

    //         accountDetails.setCust_id(custid);
    //         accountDetails.setDate_created(new Date());
    //         accountRepository.save(accountDetails);

    //         return ResponseEntity.ok("Account created successfully for customer ID: " + custid);
    //    }else {
    //         return ResponseEntity.badRequest().body("Customer with ID: " + custid + " does not exist.");
    //    }
    // }
